...
func (r *ResponseRouter) Handle(request ziface.IRequest) {
	...
	if targetPIDStr := env.ConnTags[connkey.TagTargetPlayerID]; targetPIDStr != "" {
		targetPID, _ := strconv.ParseInt(targetPIDStr, 10, 64)
		val, ok := r.GW.PlayerConns.Load(targetPID)
		if !ok {
			zlog.Ins().ErrorF("ResponseRouter: target player not connected: %s", targetPIDStr)
			return
		}
...
func (r *ResponseRouter) applyConnTags(conn ziface.IConnection, tags map[string]string) {
	...
	if pidStr, ok := tags[connkey.PropPlayerID]; ok {
